...
const enterNumText = `Please enter a number than zero`;
...
while (restartGame){
    ...
    // Verifies the user's entry for a range number is a number greater than zero (NOTE: All numbers, positive and negative, have a default boolean value of true, exept for zero which has a default Boolean value of false. Also, NaN has a default bollean value of false as well)
    while (!rangeNum || rangeNum < 1){
        rangeNum = parseInt(prompt(enterNumText));
    }

    // Creates the random number using the range number enetered by the user
    ...
    // Continues looping until the user guesses the correct number of runs out of attempa (loops until a BREAK keyword is run)
    while (true){
        // Displays the number/answer when a code word is entered
        if (guess === `Mellon`){ 
            alert(`The number is ${randomNum}`);
            guess = prompt(`Please enter a guess between 1 and ${rangeNum}. You have ${attempts} attemp(s) left:`);
        }
        ...
        //  Checks if the user guessed correctly. If so, the game ends.
         if (guess === rangeNum){
            ...
            \nThe number was ${randomNum}`);
             break;
         // Checks if user's guess was too low and promts user too guess again if guess was too low
    ...
    // Loop continues until user submits a calid response
    while (true) {
